    ...
    <section className="mt-16 w-full max-w-xl">
      ...
      <div className="flex flex-col w-full gap-2">
        <form className="relative flex justify-center items-center" onSubmit={handleSubmit}>
          <img src={linkIcon} alt="link_icon" className="left-0 absolute my-2 ml-3 w-5" />
        ...
        <div className="flex flex-col gap-1 max-h-60 overflow-y-auto">
          {
            allArticles.map((item,index) => (
              <div key={`link-${index}`} onClick={() => setArticle(item)} className="link_card">
                  ...
                  <p className="flex flex-1 font-satoshi font-medium text-sm text-blue-500 truncate">
                    {item.url}
                  </p>
              </div>
            ))
      ...
      <div className="my-10 max-w-full flex justify-center items-center">
          {
            ...
            error ? (
            <p className="font-inter text-black font-bold text-center">Well that wasn't supposed to happen.<br /><span className="font-satoshi font-normal text-gray-700">{error?.data?.error}</span></p> ) :
            (
              article.summary && (
